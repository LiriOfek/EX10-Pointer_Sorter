...
#define PRINT_POINTER_ARRAY_AFTER_SORT_BY_ADDRESS ("\npointer array after sort array by addresses:\n")
...
void main() {
	...
	*					 print the index in array, the address, and the value
	*					 call function that sort pointer array addresses
	*					 and print the index, address and value of each pointer
	*
	* Parameters - no Input/Output parameters
	*
	* Return Value - there isn't return value
	...
	* Author - Liri
	...
	/*sort pointer array by addrsses*/
	bubble_sort_of_addresses(pointer_array, SIZE);

	printf(PRINT_POINTER_ARRAY_AFTER_SORT_BY_ADDRESS);
	print_index_address_value_of_pointers(pointer_array);
}